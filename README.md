...
`tools/list` at `https://gateway.pipeworx.io/epa-envirofacts/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/facilities_by_zip \
  -H 'Content-Type: application/json' \
  -d '{"zip":"94704"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/facilities_by_zip`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "epa-envirofacts": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-epa-envirofacts"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-epa-envirofacts
```
